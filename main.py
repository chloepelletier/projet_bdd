...
def getJoueur(id=None, saison = None):
    ...
    cmd02 = 'SELECT DISTINCT annee FROM participe WHERE id_joueur=' + id + ' ORDER BY annee DESC;'
    cur.execute(cmd02)
    ...
    if(saison == "Carriere") :
        ...
        cmd01 = 'SELECT MAX(date_partie) FROM partie WHERE annee ='+saison+';'
        cur.execute(cmd01)
        date = str(cur.fetchone()[0])

    cmd2 = "SELECT * FROM contrat WHERE id_joueur="+id+" AND debut_incl <= '"+date+"' AND '"+date+"' < fin_excl;"
    cur.execute(cmd2)
    info2 = cur.fetchone()
    if (info2) :
    ...
    if (saison != "Carriere"):
        ...
        cmd4="SELECT COUNT(*), SUM(P.minutes) FROM participe P WHERE P.id_joueur =" +id+ " AND P.annee="+saison+";"
        cur.execute(cmd4)
        ...
        if(nb_partie) : #pour éviter les divisions par 0
            ...
            cmd5='SELECT COUNT(*) FROM  assiste A WHERE A.annee='+saison+' AND A.id_joueur='+id+';'
            cur.execute(cmd5)
            nb_assiste = round(cur.fetchone()[0],2)
            #action
            cmd6 = 'SELECT * FROM  action A WHERE A.annee='+saison+' AND A.id_joueur='+id+';'
            cur.execute(cmd6)
            action = cur.fetchall()
        ...
        COUNT(IF(A.type_action='lancer' AND
            ...
            ,1,NULL)) AS panier3pt
        FROM action A, joueur J, participe P, (SELECT COUNT(*) as nb_partie FROM participe WHERE id_joueur = '''+id+ ''') as P2
        WHERE A.annee=P.annee AND A.num_partie=P.num_partie AND A.id_joueur = P.id_joueur AND J.id_joueur=P.id_joueur AND P.id_joueur = '''+id+'''
        ...
        if(nb_partie): #pour éviter les divisions par 0
            ...
            moy_vol_balle = stats[5] / nb_partie
            moy_revirement = stats[6] / nb_partie
